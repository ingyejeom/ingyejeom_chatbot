...
@router.post("/ingest")
async def ingest_file(request: Request, file: UploadFile = File(...), space_id: str = Form(...), user_id: str = Form("Unknown")):
    save_path = f"/app/data_storage/{file.filename}"
    with open(save_path, "wb") as buffer:
        content = await file.read()
        buffer.write(content)
    
    return await process_ingest(save_path, space_id, request.app, user_id)